...
class Welcome extends CI_Controller
{
	...
	public function login()
	{
		...
		if (isset($user) and isset($pas)) {
			...
			foreach ($data["usuarios"] as $usu) {
				...
				if (($usu->correo == $user) and ($usu->contrasenia == $pass)) {
					// code...
					$newdata = array(
					...
					header("Location: http://127.0.0.1:8888/OrganizadoDeTareas/index.php/welcome");
					die();
				} else {
	...
	public function Renewpassword()
	{
		...
		$datos = $this->model_login->validar_correo($validarcorreo);
		//var_dump($datos);
	...
	public function  send($correo, $generado)
	{
		...
		$enviar = $this->load->view('plantilla_correo_contrasenia', $data, TRUE);
	...
	public function newpassword()
	{
		...
		$actualizar = $this->model_login->password_update($pass, $nombre, $correo);
